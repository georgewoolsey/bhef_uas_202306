# UAS Tree Spatial Arrangement and ICO Example{#tree_spatial}
...
plt_grps_temp = ortho_plt_fn(my_suid) +
    ...
    scale_color_viridis_d(option="mako", direction = -1, drop = F) + 
    labs(color = "") +
    theme(legend.key = element_rect(color = NA, fill = NA), legend.margin = margin(6.5,0,6.5,0)) +
    guides(size = "none", color = guide_legend(override.aes = list(size = 5)))
...
#### Create function to get tree list with clumps{#clump_trees}
...
plt_fnl_temp = 
  ggplot() + 
    ...
    scale_fill_viridis_d(option="mako", direction = -1, name = "clump size", drop = F) + 
    # tree points
...
# desired proportion (%) of trees in each clump size
  ...
  # c(.18, .33, .24, .10, .15)
  target_pcts = c(.18, .33, .24, .10, .15, 0)
########################################################################################
########################################################################################
# desired BA, QMD, and proportion of trees in each clump size
########################################################################################
...
get_target_check_prescription = function(
  ...
  #############################################
    target_data = 
      ...
      dplyr::tibble(
        suid = rep(clump_n_trees_grp_summary_dta$suid[1],6)
...
```{r}
# clump_polys_temp %>% dplyr::filter(clump_n_trees_grp == "16-25 trees") %>% mapview()
###### !!!!!!!!!!!!!!!!! CUT BY NARROW AREAS?
# just one clump example
id_temp = # 141
  clump_polys_temp %>% 
  dplyr::filter(clump_n_trees_grp == "16-25 trees") %>% 
...
example for random sample of trees
...
ggplot() +
  ...
  geom_sf(
    data = rand_trees %>% dplyr::filter(is_keep_tree==0)
    , mapping = aes(color = "cut tree")
    , size = 0.7
  ) +
  scale_fill_viridis_d(option="mako", direction = -1, name = "clump size", drop = F) + 
...
cut_grp_ex_temp = ttops_temp %>%
  dplyr::filter(clump_n_trees_grp == ">25 trees") %>%
  dplyr::ungroup()
...
ggplot() + 
    ...
    geom_sf(
      data = clump_polys_temp %>% dplyr::filter(clump_n_trees_grp == "16-25 trees")
    ...
    scale_fill_manual(
      values = 
        viridis::turbo(
          clump_polys_temp %>% dplyr::filter(clump_n_trees_grp == "16-25 trees") %>% 
          dplyr::pull(clump_id) %>% 
          unique() %>% length()
        ) %>% sample()
    ) + 
    ...
    labs(color = "is keep tree?", caption = "*existing clumps distinguished by color", subtitle = "cut trees in clumps sized 16-25 trees") +
    theme_void() + 
...
# plot it
  ggplot() + 
    ...
    scale_color_manual(values = c("red"), name = "") +
    labs(
      subtitle = "cut trees in clumps sized 16-25 trees"
      # caption = "*cut trees shown in red"
    ) +
    theme_void() +
...
plt_fnl_temp2
# save it
ggplot2::ggsave("../data/NSW_21.jpeg", dpi = "print", height = 11, width = 5.8, device = "jpeg")
```
...
plt_open_temp2
# save it
ggplot2::ggsave("../data/NSW_22.jpeg", dpi = "print", height = 11, width = 5.8, device = "jpeg")
```
...
# save it
ggplot2::ggsave("../data/NSW_23.jpeg", dpi = "print", height = 11, width = 8, device = "jpeg")
```
...
# save it
ggplot2::ggsave("../data/NSW_24.jpeg", dpi = "print", height = 11, width = 8, device = "jpeg")
```
...
# save it
ggplot2::ggsave("../data/NSW_25.jpeg", dpi = "print", height = 11, width = 8, device = "jpeg")
```
...
clump_n_trees_grp_summary_temp2 %>% 
  ...
  kableExtra::kbl(
    digits = 1
    , escape = F
    , caption = paste0("Pre- and Post-treatment overstory tree clump summary<br>", "stand suid: ", clump_n_trees_grp_summary_temp$suid[1])
    , col.names = c(
      ".", ""
      , "trees"
      , "mean<br>DBH (in)"
...
## Full ICO Prescription Process{#implement}
...
* identify current stand clump groupings:
  + [clump individual trees](#clump_trees)
  + generate clump [vectors](#clump_sum) and [summary metrics](#clump_metrics)
* [set ICO targets](#set_targets)
* [implement the prescription](#cut_clump_ex)
  + start with the largest clump size currently with trees
  + cut trees to the next largest clump size until desired # clumps is reached
  + repeat with each successive clump size through to individual tree selection
  + if possible, cut in same clump until desired proportions are reached to minimize machine time
...
######################################################################
######################################################################
# below here is old stuff in which I figured out some methods 
# that I might want to use sometime in the future
######################################################################
######################################################################
```
      ...
      x %>% 
        ...
        dplyr::mutate(
          ...
          , clump_n_trees_grp = cut(
            ...
            factor(
              ordered = T
              , levels = c("Individual","2-4 trees","5-9 trees","10-15 trees","16-25 trees",">25 trees")
            )
        ) %>% 
        dplyr::ungroup()
