...
if( 
  ...
  # determin keep/cut for the remaining trees in that group type 
  remaining_trees = 
    ttops_temp %>%
      dplyr::ungroup() %>% 
...
###############################################
# now process to go the next groups
###############################################
while(grp_temp>sm_grp_temp){stop("fix me plz")}
  # get the next group
  grp_temp = target_data_temp %>%
    dplyr::filter(clump_n_trees_grp!=grp_temp) %>% 
    dplyr::arrange(desc(clump_n_trees_grp)) %>% 
  ...
  if(
    ...
    # determine group size of remaining trees
    potential_trees = 
      ttops_temp %>% 
        dplyr::ungroup() %>% 
        dplyr::inner_join(
          remaining_trees %>% dplyr::filter(is_keep_tree == 1)
          , by = "treeID"
        ) %>% 
        sp_clump_points(clump_dist_m = tree_clump_dist_m) %>% 
