...
# 2) get angle of f line 
  # st_ends_heading(f_line_temp)["head"]
    # angle_fn_temp = function(stcoord1, stcoord2){
    #   # a = 90 - 180 * atan(
    #   #   (stcoord2["Y"]-stcoord1["Y"]) / # RISE
    #   #     (stcoord2["X"]-stcoord1["X"]) # RUN
    #   # ) / pi
    #   # alternative?
    #   a = atan(
    #     (stcoord2["Y"]-stcoord1["Y"]) / # RISE
    #       (stcoord2["X"]-stcoord1["X"]) # RUN
    #   )
    #   if(a<0){a = a + pi * 2} # make negative angles positive by adding 360 degrees
    #   a = a * ( 180 / pi ) # convert angle from radians to degrees
    #   ## return
    #   return(a-180)
    # }
    # angle_fn_temp(sf::st_coordinates(f_pts_temp)[1,], sf::st_coordinates(f_pts_temp)[2,])

# 3) make a line perpendicular to our farthest line based on a point on the f line
perp_line = function(stcoord1, angle, dist_m, crs = sf::st_crs(x)){
  # get new coordinate 1
  y1 = (dist_m/2) * cos(90 + angle)
  x1 = (dist_m/2) * sin(90 + angle)
  stcoord3 = ( stcoord1 + c(x1, y1) ) %>% 
    sf::st_point() %>% 
    sf::st_sfc()
  sf::st_crs(stcoord3) = crs
  # get new coordinate 2
  y1 = -(dist_m/2) * cos(90 + angle)
  x1 = -(dist_m/2) * sin(90 + angle)
  stcoord4 = ( stcoord1 + c(x1, y1) ) %>% 
    sf::st_point() %>% 
    sf::st_sfc()
  sf::st_crs(stcoord4) = crs
  # draw a line
  l = sf::st_union(stcoord3,stcoord4) %>% 
    sf::st_cast("LINESTRING") %>% 
    sf::st_as_sf()
  return(l)
}
...
perp_line_sf_temp = 1:length(line_pts_temp) %>% 
  purrr::map(function(xxx){
    return(
      perp_line(
        sf::st_coordinates(line_pts_temp)[xxx,]
        , angle = st_ends_heading(f_line_temp)["head"]
          # angle_fn_temp(sf::st_coordinates(f_pts_temp)[1,], sf::st_coordinates(f_pts_temp)[2,])
        , dist_m = sf::st_length(f_line_temp) %>% as.numeric() # %>% `*`(2)
        , crs = sf::st_crs(x)
      ) %>% 
      sf::st_set_geometry("geometry") %>% 
      dplyr::mutate(line_n = xxx)
  )}) %>% 
  dplyr::bind_rows()
...
# 7) make cuts at the points where there is the least overlap with the clump polygon
  ...
  cut_tree_lines_temp = 
    x %>% 
      ...
      dplyr::arrange(len_m, line_n, treeID) %>% 
...
**put that entire process into a function to use with the clump tree list which we'll nest by clump size**
...
# 1) points along the f line
  line_pts_temp = f_line_temp %>% 
    sf::st_line_sample(
      # n = # number of points = length line / (d/2)
      #   sf::st_length(f_line_temp) %>% 
      #   as.numeric() %>% 
      #   `/`(tree_clump_dist_m/2) %>% 
      #   ceiling()
      , density = 1 # density (points per distance unit) of the sampling: 2=0.5m
...
perp_line(
  stcoord1 = sf::st_coordinates(line_pts_temp)[15,]
  , angle = st_ends_heading(f_line_temp)["head"]
    # angle_fn_temp(sf::st_coordinates(f_pts_temp)[1,], sf::st_coordinates(f_pts_temp)[2,])
  , dist_m = sf::st_length(f_line_temp) %>% as.numeric() %>% `*`(2)
  , crs = sf::st_crs(x)
...
perp_line_sf_temp = 1:length(line_pts_temp) %>% 
  purrr::map(function(xxx){
    return(
      perp_line(
        sf::st_coordinates(line_pts_temp)[xxx,]
        , angle = angle_fn_temp(sf::st_coordinates(f_pts_temp)[1,], sf::st_coordinates(f_pts_temp)[2,])
        , dist_m = sf::st_length(f_line_temp) %>% as.numeric() # %>% `*`(2)
        , crs = sf::st_crs(x)
      ) %>% 
      sf::st_set_geometry("geometry") %>% 
